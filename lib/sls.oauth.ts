...
export function InitSlsOauth(config: SlsOauthConfig): Promise<SlsOauth> {
  ...
  return slsOauth.Start()
    .then(() => { return slsOauth; })
...
export class SlsOauth {
          ...
          this.refreshAccessToken(uuid)
            ...
            .catch((err: any) => {
              if (err.response !== undefined)
                log.error(`[${uuid}] refreshAccessToken failed with status ${err.response.status}: ${JSON.stringify(err.response.data)}`);
            });
  ...
  private refreshAccessToken(uuid: string): Promise<void> {
    return this.generateAccessToken(uuid)
      .then((token: string) => { this.accessToken = token; })
      .catch((err: any) => { throw err; })
  }

  private generateAccessToken(uuid: string): Promise<string> {
    return this.post(uuid, '/oauth2/token', {
      data: qs.stringify({
  ...
  private request(
    ...
    auth: boolean = true): axios.AxiosPromise {
    ...
    if (auth)
      config.headers[AUTHORIZATION_HEADER] = `Bearer ${this.accessToken}`;
    ...
    if (this.config.verbose)
      log.debug(`[${uuid}] ${config.method} ${url}`);

    else if (this.config.debug)
      log.debug(`[${uuid}] ${config.method} ${url} ${config.data == undefined ? '' : JSON.stringify(config.data)}`);
    ...
    return this.instance.request(config)
      .then((resp: axios.AxiosResponse) => {
        ...
        if (this.config.verbose)
          log.debug(`${str} (${duration})`);

        else if (this.config.debug)
          log.debug(`${str} ${resp.data == undefined ? '' : JSON.stringify(resp.data)} (${duration})`);
      ...
      .catch((err: any) => {
        if (err.response != undefined) {
          if (err.response.status === 429 || err.response.status === 401) {
            ...
            log.warn(`[${uuid}] Oauth request to ${url} exceeded the API's limit. Retry in ${timeout / 1000} seconds...`);
  ...
  private requestAgainIn(
    uuid: string,
    config: axios.AxiosRequestConfig,
    timeout: number): axios.AxiosPromise {

    return new Promise<axios.AxiosPromise>((resolve) => {
      setTimeout(() => { resolve(); }, timeout);
    })
      .then(() => { return this.request(uuid, config); })
      .catch((err: any) => { throw err; });
  }
